...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (finalPayload) {
          // Fix: Explicitly cast 'received' as const to match TriggerLog status type
          setTriggerLogs(prev => [{ 
            timestamp: Date.now(), 
  ...
  const handleAutomationWorkflow = async (payload: string) => {
    ...
    if (cmd === 'start') {
      ...
      // Fix: cast status literal to union type
    ...
    audio.onplay = () => {
       // Fix: cast status literal to union type
       setTriggerLogs(prev => prev.map((log, i) => i === 0 ? { ...log, status: 'playing' as const } : log));
    };
    ...
    audio.onerror = () => {
      ...
      // Fix: cast status literal to union type
      setTriggerLogs(prev => prev.map((log, i) => i === 0 ? { ...log, status: 'error' as const, errorDetails: 'Format/CORS' } : log));
      if (status === RecorderStatus.IDLE || status === RecorderStatus.ERROR) {
        toggleListen(false);
      }
    };
